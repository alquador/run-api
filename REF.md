...
```sh
curl --include --request POST http://127.0.0.1:27017:8000/sign-up \
...
```sh
curl --include --request POST http://127.0.0.1:27017:8000/sign-in \
  --header "Content-Type: application/json" \
  --data '{
    "credentials": {
      "email": "an@example.email",
...
```sh
curl --include --request PATCH http://127.0.0.1:27017:8000/change-password/ \
...
```sh
curl --include --request DELETE http://127.0.0.1:27017:8000/sign-out/ \
  --header "Authorization: Bearer $TOKEN"
```
